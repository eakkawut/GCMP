/*---------------------------------------------------------------------------------------------
 *  MoonshotAI Dedicated Provider
 *  Provides multi-key management and dedicated configuration wizard functionality for MoonshotAI provider
 *--------------------------------------------------------------------------------------------*/
...
/**
 * MoonshotAI Dedicated Model Provider Class
...
export class MoonshotProvider extends GenericModelProvider implements LanguageModelChatProvider {
    ...
    /**
     * Static Factory Method - Create and Activate MoonshotAI Provider
     */
    static createAndActivate(
        ...
        // Register command to set Moonshot API key
        ...
        // Register command to set Kimi API key
    ...
    /**
     * Override: Convert ModelConfig to LanguageModelChatInformation
     * When model's provider is "kimi", display provider name as "Kimi"
     */
    protected override modelConfigToInfo(model: ModelConfig): LanguageModelChatInformation {
        ...
        // If model uses kimi provider, modify displayed provider name
    ...
    private async ensureApiKeyForModel(modelConfig: ModelConfig): Promise<string> {
        ...
        if (isKimi) {
            // Kimi For Coding model directly enters dedicated key setup
            await MoonshotWizard.setKimiApiKey(this.providerConfig.displayName, this.providerConfig.codingKeyTemplate);
        } else {
            // Moonshot model directly enters normal key setup
    ...
    async provideLanguageModelChatResponse(
        ...
        Logger.debug(
            `${this.providerConfig.displayName}: About to process request, using ${providerKey === 'kimi' ? 'Kimi For Coding' : 'Moonshot'} key - Model: ${modelConfig.name}`
        );
        ...
        try {
            ...
            // Based on key type used, delayed update corresponding status bar usage
            if (providerKey === 'kimi') {
