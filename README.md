# CCMP - 提供多个国内原生大模型提供商支持的扩展

[![CI](https://github.com/VicBilibily/CCMP/actions/workflows/ci.yml/badge.svg)](https://github.com/VicBilibily/CCMP/actions)
[![License](https://img.shields.io/badge/License-MIT-orange)](https://github.com/VicBilibily/CCMP/blob/main/LICENSE)
...
- **0.20.8 临时兼容项**：针对 VS Code 模型选择器显示模型不全的问题，新增 `ccmp.autoPrefixModelId` 配置项作为临时解决方案。
    - **问题原因**：VS Code 1.111.0 引入模型折叠模式后，当多个提供商存在相同模型 ID 时，选中模型后的可用模型过滤逻辑异常（选中后模型移至顶部，下方列表过滤掉该 modelId，导致同 modelId 的其他提供商的此个模型无法选中）。
    - **解决方案**：启用 `ccmp.autoPrefixModelId` 后，模型 ID 将自动添加提供商前缀（如 `zhipu:::glm-5`），避免 ID 冲突。（若仅使用单一提供商或未遇到此类问题，可无视此临时兼容项）
...
在VS Code扩展市场搜索 `CCMP` 并安装，或使用扩展标识符：[`guokoko.ccmp`](https://marketplace.visualstudio.com/items?itemName=guokoko.ccmp)
...
CCMP 支持通过 VS Code 设置来自定义AI模型的行为参数，让您获得更个性化的AI助手体验。
...
```json
{
    "ccmp.maxTokens": 16000, // 32-256000
    "ccmp.retry.maxAttempts": 3, // 1-5，仅对可重试错误生效
    "ccmp.zhipu.search.enableMCP": true // 启用`联网搜索MCP`（Coding Plan专属）
}
```

- `ccmp.retry.maxAttempts` 默认值为 `3`，用于控制 429、限流和临时过载类错误的最大自动重试次数。
...
CCMP 支持通过 `ccmp.providerOverrides` 配置项来覆盖提供商的默认设置，包括 baseUrl、customHeader、模型配置等。
...
CCMP 提供 **OpenAI / Anthropic Compatible** Provider，用于支持任何 OpenAI 或 Anthropic 兼容的 API。通过 `ccmp.compatibleModels` 配置，您可以完全自定义模型参数，包括扩展请求参数。

1. 通过 `CCMP: Compatible Provider 设置` 命令启动配置向导。
2. 在 `settings.json` 设置中编辑 `ccmp.compatibleModels` 配置项。
...
`ccmp.compatibleModels[*].sdkMode` 用于指定兼容层的请求/流式解析方式。除 `openai` / `anthropic` 标准模式外，以下两项为**实验性**能力：
...
> **使用前必读**：需先在对话模型中配置并验证 ApiKey；在输出面板选择 `GitHub Copilot Inline Completion via CCMP` 可查看调试信息。接入的是通用大模型，**未针对代码补全专门训练**，效果可能不及 Copilot 原生 Tab 补全。
...
FIM 和 NES 补全都使用单独的模型配置，可以分别通过 `ccmp.fimCompletion.modelConfig` 和 `ccmp.nesCompletion.modelConfig` 进行设置。
...
```json
{
    "ccmp.fimCompletion.enabled": true, // 启用 FIM 补全功能
    "ccmp.fimCompletion.debounceMs": 500, // 自动触发补全的防抖延迟
    "ccmp.fimCompletion.timeoutMs": 5000, // FIM 补全的请求超时时间
...
````json
{
    "ccmp.nesCompletion.enabled": true, // 启用 NES 补全功能
    "ccmp.nesCompletion.debounceMs": 500, // 自动触发补全的防抖延迟
    "ccmp.nesCompletion.timeoutMs": 10000, // NES 补全请求超时时间
    "ccmp.nesCompletion.manualOnly": true, // 启用手动 `Alt+/` 快捷键触发代码补全提示
...
```json
{
    ...
    "ccmp.fimCompletion.enabled": true,
    "ccmp.fimCompletion.debounceMs": 500,
    "ccmp.fimCompletion.timeoutMs": 5000,
    "ccmp.fimCompletion.modelConfig": {
        "provider": "mistral",
...
CCMP 提供上下文窗口占用比例状态栏显示功能，帮助您实时监控当前会话的上下文窗口使用情况。
...
CCMP 内置了完整的 Token 消耗统计功能，帮助您追踪和管理 AI 模型的使用情况。
...
- **查看统计**：点击状态栏的 Token 用量显示，或通过命令面板执行 `CCMP: 查看今日 Token 消耗统计详情` 命令
...
```json
{
    "ccmp.usages.retentionDays": 100 // 历史数据保留天数（0表示永久保留）
}
```
...
CCMP 支持在提交前自动读取当前仓库的改动（已暂存/未暂存/新文件），提取关键 diff 片段并结合相关历史提交与仓库整体提交风格（auto 模式下）来生成更贴合你项目习惯的提交信息。
...
> 多仓库工作区：如果当前工作区包含多个 Git 仓库，CCMP 会尝试根据你点击的 SCM 区域推断仓库；无法推断时会弹出仓库选择。

### 模型选择与配置

该功能基于 **VS Code Language Model API** 调用模型。

- 第一次使用或未配置模型时，会自动引导选择模型（也可手动运行 `CCMP: 选择 Commit 消息生成模型`）。
- 相关配置项：

```json
{
    "ccmp.commit.language": "chinese", // 生成语言：chinese / english（auto 模式语言不明确时的回退值）
    "ccmp.commit.format": "auto", // 提交消息格式：auto(默认) / 见下方 format 说明
    "ccmp.commit.customInstructions": "", // 自定义指令（仅当 format=custom 时生效）
    "ccmp.commit.model": {
...
### `ccmp.commit.format` 格式说明与示例

> 说明：以下示例仅用于展示格式形态；实际内容会根据你的 diff 自动生成。

- `auto`：自动推断（会参考仓库历史的语言/风格；不明确时回退为 `plain` + `ccmp.commit.language`），默认推荐。

- `plain`：简洁一句话，不含 type/scope/emoji（适合快速提交）。

- `custom`：完全由你的自定义指令控制（`ccmp.commit.customInstructions`）。
...
```bash
# 克隆项目
git clone https://github.com/VicBilibily/CCMP.git
cd CCMP
# 安装依赖
npm install
